  ...
  ```javascript
{
    ...
    // Use the maxRedirects parameter to set the maximum number of redirects. The default value is undefined.
    maxRedirects: 5,

    // Use the interceptorChain parameter to set the system-level interceptor. The default value is undefined.
...
Set up [System-level interceptor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-http#httpinterceptor22)(The interceptor built through the system network API; can intercept redirection requests; requires API >= 2)
```javascript
...
> **NOTE**:<br>
> 1.The certificate-based bidirectional authentication(clientCert) and certificate pinning features are available only in API version 11+, configure RemoteValidation(RemoteValidation) are available only in API version 18+, configure system-level interceptors(interceptorChain) are available only in API version 22+,configure maxRedirects(maxRedirects) are available only in API version 23+, Other features are available since API version 9.<br>
